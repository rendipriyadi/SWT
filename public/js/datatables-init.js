$(document).ready(function() {
    ...
    // Mobile Reports Table (simplified columns with collapse) - INDEPENDENT initialization
    ...
    // ------- Reports Date Range - Elegant Datepicker (no Flatpickr) -------
    function initReportsDateRange(){
        const $start = $('#report_created_start');
        const $end = $('#report_created_end');
        if (!$start.length || !$end.length) return;
        
        // Enable/disable end date based on start date
        $start.on('change changeDate', function(){
            const startVal = $start.val();
            if (startVal){
                $end.prop('disabled', false);
                // Set min date for Bootstrap Datepicker
                if ($end.data('datepicker')){
                    $end.datepicker('setStartDate', new Date(startVal));
                }
    ...
    // Initialize after Elegant Datepicker converts inputs
    setTimeout(initReportsDateRange, 600);
    ...
    function handleReportApply(e){
        ...
        // On change: only update Reset button visibility (constraints handled by initReportsDateRange)
    ...
    if ($('#sejarahTable').length) {
        ...
        // ------- History Date Range - Elegant Datepicker (no Flatpickr) -------
        ...
        function toggleHistoryResetButton() {
            ...
            if (!btn) return;
            const hasAny = Boolean(($('#history_created_start').val() || $('#history_created_end').val()) || (window.sejarahDateFilter && (window.sejarahDateFilter.start || window.sejarahDateFilter.end)));
            if (hasAny) btn.classList.remove('d-none'); else btn.classList.add('d-none');
        }
        
        // Initialize after Elegant Datepicker converts inputs
        setTimeout(initHistoryDateRange, 600);
        toggleHistoryResetButton();
